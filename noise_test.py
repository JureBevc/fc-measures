...
for noise_amount in noise_amounts:
    ...
    pearson_results.append(pearson.pearson(
        np.array([original_signal, signal_noised]))[0][1])
...
covariance_results = []
for noise_amount in noise_amounts:
    signal_noised = original_signal + np.random.normal(0, noise_amount, len(original_signal))
...
for noise_amount in noise_amounts:
    signal_noised = original_signal + \
        np.random.normal(0, noise_amount, len(original_signal))
    icov_results.append(
        icov.ICOV(np.array([original_signal, signal_noised]))[0][1])
...
for noise_amount in noise_amounts:
    signal_noised = original_signal + \
        np.random.normal(0, noise_amount, len(original_signal))
    cross_correlation_results.append(cross_correlation.cross_correlation(
        np.array([original_signal, signal_noised]))[0][1])
...
for noise_amount in noise_amounts:
    signal_noised = original_signal + \
        np.random.normal(0, noise_amount, len(original_signal))
    coherence_results.append(coherence.coherence(
        np.array([original_signal, signal_noised]))[0][1])
